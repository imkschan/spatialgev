...
savefit <- TRUE
fname <- 'real_gauss_2015_bhm.RData'
...
for (k in 1:tt){
	mdata[,,k] <- getwm_oneyear(dlr1[k,,],dt,yr)
}
site <- t(siteloc)
...
for (k in 1:totalType){
	...
	loc.t[1,,,k] <- mvrnorm(n=n,mu=tmpmu,Sigma=tmpSigma)
	# loc.t[1,,,k] <- loc.p[,,k] <- raw.loc[,,k,r]
...
if (step > 1) for (i in 2:step){
	...
	if (i %% 100 == 0) {
		...
		ii <- i-1
		cat('\n',i,' ',sapply(1:nblock, function(x) mean(us[which(bin[[bb]]<=(ii)),x]<=al[which(bin[[bb]]<=(ii)),x])),' ',loc.ac/(i*nb),'\n')
	...
	for (k in 1:totalType){
		...
		if (i==2) kn.bbloc[,,k] <- (diag(c(bloc[1,k] * 0.15, beta1[1,k] * 0.1, beta2[1,k]) * 0.1)*0.3)^2
		...
		b <- k + totalType
		if (i==2) kn.gsigma[,,k] <- (diag(c(gvar[1,k] * 0.015, gsc[1,k] * 0.055))*1.2)^2
		if (i==(cutoff1+1)) {
			# kn.gsigma[,,k] <- var(cbind(gvar[bin1,k], gsc[bin1,k])) * 2
		}
		if (i==(cutoff2+1)) {
			kn.gsigma[,,k] <- var(cbind(gvar[bin2,k], gsc[bin2,k]) ) * 1.3
		}
		...
		if (p.gsigma[1] > 0 && p.gsigma[2] > 0){
			...
			l.p[b] <- loglikgau2(loc.t[loci.1,,,k],site,tmpmu,pSigma)
			# tmplik <- loglikgau(loc.t[i-1,,,k],site,bloc[i,k],gvar[i-1,k],gsc[i-1,k])
		} else l.p[b] <- -Inf
		al[i,b] <- rdiff(l.p[b],tmplik)
		us[i,b] <- runif(1)
		if (us[i,b] <= al[i,b]) {
		...
		if (i==2) 
			if (k==2)
				{kn.loc[,,k] <- diag(x=(0.02)^2,nrow=(nsite),ncol=(nsite))}
			else
				{kn.loc[,,k] <- diag(x=(0.05)^2,nrow=(nsite),ncol=(nsite))}		
		# if (i==(cutoff1+1)) kn.loc[,,k] <- loc.t[x,bin1,k] * 2
		# if (i==(cutoff2+1)) kn.loc[,,k] <- loc.t[bin2,,,k] * 2
		
		if (all.loc.t==TRUE){
			if (i==(cutoff2+1)) kn.loc[,,k] <- getloccov(loc.t[,,,k],bin2) * 0.1
		} else {
		...
		for (p in 1:nb){
			...
			# tmplik <- llgev1(mdata[x,,k],loc.t[x,,k],sc[i-1,k],sh[i-1,k])
		...
		# loc.t[i,,,k] <- loc.t[loci.1,,,k]
		loc.sample[i,k] <- loc.t[loci,1,1,k]

		b <- k + 2 * totalType
		if (i==2) kn.gev[,,k] <- diag((c(sc[1,k] * 0.013,sh[1,k] * 0.09) * 1.1 )^2,nrow=2,ncol=2)
		if (i==(cutoff1+1)) {
...
if (savefit==TRUE){
	save(n,yr,dimcop,lengthcop,tt,totalType,nsite,site,
		bloc,loc.sample,loc.t,sc,sh,gvar,gsc,copparm,
